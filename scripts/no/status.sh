...
status()
{
	...
	while true; do
		...
		local balance=$(node $installdir/console.js --substrate-ws-endpoint "wss://para2-api.phala.network/ws/" chain free-balance $gas_address 2>&1)
		balance=$(echo $balance | awk -F " " '{print $NF}')
		balance=$(echo $balance / 1000000000000 | bc)
		local node_block=$(curl -sH "Content-Type: application/json" -d '{"id":1, "jsonrpc":"2.0", "method": "system_syncState", "params":[]}' http://node.phala.asia:9933 | jq '.result.currentBlock')
