...
check_version()
{
	...
	if [ "$(cat $installdir/.env | awk -F "=" '{print $NF}')" != "$(cat /tmp/phala/solo-mining-scripts-para/.env | awk -F "=" '{print $NF}')" ]; then
		...
		cp /tmp/phala/solo-mining-scripts-para/{.env,console.js,docker-compose.yml.no} /opt/phala
		mv /opt/phala/docker-compose.yml.no /opt/phala/docker-compose.yml
		cp -r /tmp/phala/solo-mining-scripts-para/scripts/no /opt/phala/scripts
