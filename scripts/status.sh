...
function status_solo_cn()
{
	...
	while true; do
		...
		local balance=$(node $installdir/console.js --substrate-ws-endpoint $khala_substrate_ws_endpoint chain free-balance $gas_address 2>&1)
		balance=$(echo $balance | awk -F " " '{print $NF}')
		balance=$(echo "$balance / 1000000000000"|bc)
		local khala_head_block=$(node $installdir/console.js --substrate-ws-endpoint $khala_substrate_ws_endpoint chain sync-state 2>/dev/null)
		...
		local kusama_head_block=$(node $installdir/console.js --substrate-ws-endpoint $kusama_substrate_ws_endpoint chain sync-state | awk -F " " '/currentBlock/ {print $NF}' | sed 's/,//g')
...
function status_node_cn()
{
	...
	while true; do
		...
		for i in ${khala_node_block} ${khala_head_block} ${kusama_node_block} ${kusama_head_block}; do
			if [ -z ${blockInfo[${i}]} ]; then
				blockInfo[${i}]=0
			fi
		done
...
function status_worker_cn()
{
	...
	while true; do
		...
		local khala_head_block=$(node $installdir/console.js --substrate-ws-endpoint $khala_substrate_ws_endpoint chain sync-state 2>/dev/null)
		khala_head_block=$(echo $khala_head_block | awk -F "," '{print $5}' | sed 's/ currentBlock: //g')
		local node_ip=$(cat $installdir/.env | grep 'NODE_ADDRESS' | awk -F "=" '{print $NF}')
		local khala_node_block=$(curl -sH "Content-Type: application/json" -d '{"id":1, "jsonrpc":"2.0", "method": "system_syncState", "params":[]}' http://${node_ip}:9933 | jq '.result.currentBlock')
		local kusama_head_block=$(node $installdir/console.js --substrate-ws-endpoint $kusama_substrate_ws_endpoint chain sync-state | awk -F " " '/currentBlock/ {print $NF}' | sed 's/,//g')
		local kusama_node_block=$(curl -sH "Content-Type: application/json" -d '{"id":1, "jsonrpc":"2.0", "method": "system_syncState", "params":[]}' http://${node_ip}:9934 | jq '.result.currentBlock')
		local get_info=$(curl -X POST -sH "Content-Type: application/json" -d '{"input": {}, "nonce": {}}' http://0.0.0.0:8000/get_info)
		local publickey=$(echo $get_info | jq '.payload|fromjson.public_key' | sed 's/\"//g' | sed 's/^/0x/')
		local registered=$(echo $get_info | jq '.payload|fromjson.registered' | sed 's/\"//g')
		local blocknum=$(echo $get_info | jq '.payload|fromjson.blocknum' | sed 's/\"//g')
